...
%
get_tokens(cmd,Chars,Tokens):-
  get_tokens_cmd(Chars,Tokens).

get_tokens(dat,Chars,Tokens):-
  get_tokens_dat(Chars,Tokens).

% we break in two to facilitate debugging
get_tokens_cmd(Chars,Tokens):-
  F=cmd,
  phrase(toks(F,Tokens),Chars),
  !.  % call the grammar

get_tokens_dat(Chars,Tokens):-
  F=dat,
  process_lexical_props(F),
  phrase(toks(F,Tokens),Chars),
  !.

%% process_lexical_props(+FileType) is det.
%
% Process the lexical properties of the file
% TODO: not so efficient, it should only be called once per file
process_lexical_props(cmd):-!.
process_lexical_props(dat):- % get the multiple-entry-flag
    get_value(data_file,DataFile),
    get_prop(DataFile,'multiple-entry-flag',ME),
    % Convert ME to integer
    atom_number(ME, MENumber),
    chartype(MENumber,CharName),
    set_data_flag(df8,CharName),!.

process_lexical_props(_). % do nothing if no multiple-entry-flag

% ******************************************************
%  Data flags
%  Allows for dynamic setting of data flags
% ******************************************************
set_data_flag(N, V) :-
  set_prop(data_flags, N, V).

get_data_flag(N, V) :-
  get_prop(data_flags, N, V).

clear_data_flags:-
  del_props(data_flags).
...
chartype(124,pipe):-!.  % vertical bar
chartype(Other,Other):-!. % other characters, name equal to ascii code
...
data_flag(8, Char):-
  get_data_flag(df8,Char).

data_flag(8, semicolon):-
  \+ has_prop(data_flags,df8).
...
test(get_tokens_dat_quotes):-
    ...
    format('~nLine get_tokens_dat_quotes~n  ~w~n:',[String]),
    test_lexical(Chars,TypedChars),
    format('~nTypedChars ~w~n',[TypedChars]),
...
test(get_tokens_dat_quotes_dangling):-
    ...
    string_codes(String,Chars),
    format('~nLine get_tokens_dat_quotes_dangling~n  ~w~n:',[String]),
    test_lexical(Chars,TypedChars),
    format('~nTypedChars ~w~n',[TypedChars]),
    get_tokens(dat,TypedChars,Tokens),
    format('~n Tokens~n~w~n',[Tokens]).

  test(get_token_data_flag_8):-
    clear_data_flags,
    Chars= `joaquim;manuel;costa`,
    string_codes(String,Chars),
    format('~nget_token_data_flag_8~n  ~w~n:',[String]),
    test_lexical(Chars,TypedChars),
    writeln(TypedChars),
    get_tokens(dat,TypedChars,Tokens),
    format('~nget_token_data_flag_8~n Tokens ~w~n',[Tokens]),
    member((dataflag,8),Tokens),!.

  test(get_token_data_flag_8_pipe):-
      set_data_flag(df8,pipe),
      Chars= `joaquim|manuel|costa`,
      string_codes(String,Chars),
      format('~n get_token_data_flag_8_pipe~n  ~w~n:',[String]),
      test_lexical(Chars,TypedChars),
      writeln(TypedChars),
      get_tokens(dat,TypedChars,Tokens),
      format('~n get_token_data_flag_8_pipe~n Tokens ~w~n',[Tokens]),
      member((dataflag,8),Tokens),!.

    test(get_token_data_flag_8_not_semi_colon):-
        set_data_flag(df8,pipe),
        Chars= `joaquim;manuel;costa`,
        string_codes(String,Chars),
        format('~n get_token_data_flag_8_not_semi_colon~n  ~w~n:',[String]),
        test_lexical(Chars,TypedChars),
        writeln(TypedChars),
        get_tokens(dat,TypedChars,Tokens),
        format('~n get_token_data_flag_8_not_semi_colon~n Tokens ~w~n',[Tokens]),
        \+ member((dataflag,8),Tokens),!.
  ...
  test(triple_quote):-
    ...
    string_codes(String,Chars),
    format('~n triple_quote Line  ~w~n:',[String]),
    test_lexical(Chars,TypedChars),
    format('~nTypedChars ~w~n',[TypedChars]),
    get_tokens(dat,TypedChars,Tokens),
    format('~n triple_quote Tokens ~w~n',[Tokens]),

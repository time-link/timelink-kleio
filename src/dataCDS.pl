...
%******************************************************
%    During input, groups and processing info 
%    are stored temporarily in the strucutre
%    
...
%  %
createCD:-
    setCDS(cds(_CPATH,_CGROUP,_CGROUPID,_LOCUSCOUNT,_ELEMENTLIST,_CELEMENT,
...
get_aspect(_A,E,[]):-
    getCDField(elementList,Els),
    \+ member(element(E,_L),Els). % no element %
...
getCDF(_A,E,Els,[]):-
    getCDField(elementList,Els),
    \+ member_check(element(E,_L),Els). % no element %
...
mkid_group(G,I,ID):- % see if there is an element id %
    ...
    !.
mkid_el_id(G,_I,ID):-  % check if an element is the ID %
    getCDField(elementList,E),!,% get elements of current group%
    member(element(El,_Ent),E),
    clause(clioElement(El,EID),true),% see if it has identificatio=sic %
...
mkid_entry1(_G,E,ID):-
    ...
    mult(_A) \= Core, % multiple entries not allowed in id fields %

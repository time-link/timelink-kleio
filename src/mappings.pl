:-module(mappings,[
    op(230,fx,mapping),
    op(230,fx,user_mapping),
    op(220,xfx,to),
    op(210,fx,class),
    op(210,fx,user_class),
    op(209,xfy,super),
    op(208,xfy,table),
    op(207,xfy,with),
...
%% consult_user_mappings(+File) is det
%
% Consults the user mappings file.
%
consult_user_mappings(File):-
    consult(File).

%% remove_mapping(+Map) is det
%
% Removes a mapping.
%
remove_mapping(Map):-
    retract(mapping(to(Map,_))).

%% remove_class(+Class) is det
%
% Removes a class.
%
remove_class(Class):-
      retract(class(super(Class,_))).
...
class acusacoes super object table acusacoes
    ...
    and
        literal column literal baseclass literal coltype varchar colsize 6000 colprecision 0 pkey 0
    and
        origem column origem baseclass origem coltype varchar colsize 6000 colprecision 0 pkey 0
    and
